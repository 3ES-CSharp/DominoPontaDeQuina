...
public class JogadorRepository(DominoDbContext context)
{
    ...
    /// <summary>Retorna um jogador com suas participações e as partidas relacionadas (eager loading).</summary>
    public async Task<Jogador?> GetComParticipacoesAsync(Guid id)
        => await context.Jogadores
            .AsNoTracking()
    ...
    /// <summary>Retorna os jogadores com mais vitórias, do maior para o menor total.</summary>
    public async Task<List<Jogador>> GetRankingPorVitoriasAsync(int quantidade)
        => await context.Jogadores
            .AsNoTracking()
            .OrderByDescending(j => j.Participacoes.Count(p => p.Vencedor))
            .ThenBy(j => j.NomeExibicao)
            .Take(quantidade)
            .ToListAsync();
